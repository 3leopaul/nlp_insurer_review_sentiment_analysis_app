...
def load_dataset():
    ...
    if os.path.exists(csv_path):
        ...
        if 'note' in df.columns:
            df['sentiment'] = df['note'].apply(lambda n: 'positive' if n >= 4 else ('negative' if n <= 2 else ('neutral' if n == 3 else None)))
        return df
    ...
    for filename in ["insurance_reviews_cleaned.parquet", "reviews_corrected_and_translated.parquet", "reviews_corrected.parquet"]:
        ...
        if os.path.exists(path):
            ...
            if 'avis_corrected_clean' not in df.columns and 'avis_clean' in df.columns:
                df['avis_corrected_clean'] = df['avis_clean']
            if 'note' in df.columns:
                # Only update sentiment if it's missing or if we want to correct the derived sentiment
                # For safety, let's make sure sentiment matches the note (1-2: neg, 3: neu, 4-5: pos)
                # and stays NaN/None if note is NaN
                df['sentiment'] = df['note'].apply(lambda n: 'positive' if n >= 4 else ('negative' if n <= 2 else ('neutral' if n == 3 else None)))
            return df
    return pd.DataFrame()
...
def main():
    ...
    if page == "📊 1. Exploration & Data Quality":
        ...
        with tab2:
            ...
            show_image_if_exists("ngrams_trigrams_avis_clean.png", "French Trigrams Analysis (Market Trends)")
        ...
        with tab2:
            ...
            with c1:
                ...
                # Filter notes for chart to avoid errors with NaNs or types
                plot_data = ins_df.dropna(subset=['note']).copy()
                plot_data['note'] = plot_data['note'].astype(int)
                
                if not plot_data.empty:
                    fig, ax = plt.subplots(figsize=(8, 6))
                    palette = ["#D15D5D", "#F18B52", "#9476B8", "#4D79D1", "#66CC6F"] # Semantic color mapping (red to green-ish)
                    sns.countplot(x='note', data=plot_data, hue='note', palette=palette, order=[1, 2, 3, 4, 5], ax=ax, legend=False)
                    ax.set_title(f"Score Frequency ({target})")
                    for p in ax.patches:
                        h = p.get_height()
                        if h > 0: ax.annotate(f'{int(h)}', (p.get_x() + p.get_width() / 2., h), ha='center', va='bottom')
                    st.pyplot(fig)
                else: st.warning("No rated reviews found for this insurer.")
